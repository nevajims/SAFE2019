function  [mesh,reshaped_proc_data,unsorted_results] =  SAFE_2d_modified_function (do_plot,constraint_on, solve_fem) 
...
[equispaced_points_mm, nom_el_size_mm]     =  get_outside_edge( data , 158.75 , 139.7 , 100 , do_plot); 
...
base_nodes_ind = find((mesh.nd.pos(:,2))<=min(mesh.nd.pos(:,2))*0.99);

mesh  =  get_degrees_of_freedom(mesh,base_nodes_ind ,constraint_on);



disp(['number of nodes:  ', num2str(length(mesh.nd.pos)), ',  number of elements:  ', num2str(length(mesh.el.nds))])
...
axis off;
hold on
% plot constraint nodes in here
for index = 1 :size (base_nodes_ind)
plot( mesh.nd.pos(base_nodes_ind(index),1),mesh.nd.pos(base_nodes_ind(index),2),'r.','MarkerSize',20)
end %for index = 1 :size (base_nodes_ind)
...
function mesh  =  get_degrees_of_freedom(mesh,base_nodes_ind ,constraint_on)

mesh.nd.dof   = ones(size(mesh.nd.pos, 1), 3)                           ;

if constraint_on ==1 

for index = 1:length(base_nodes_ind)
mesh.nd.dof(base_nodes_ind(index),:) = [0,0,0];  
end %for index = 1:length(base_nodes_ind)

end % if constraint_on ==1 

end %function mesh  =  get_degrees_of_freedom(mesh, )
