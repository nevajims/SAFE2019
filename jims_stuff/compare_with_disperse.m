function []  =  compare_with_disperse (lite_index)
% --------------------------------------------
% have a better way of interpolating so that the full range of the dispersion curve can be used
% Solve the problem with interpolation of each mode -- what is the problem
%---------------------------------------------------------------------------------
% find where the gradient goes very steep for each mode- then restrict the--------
% search to a certain percentage above this --------------------------------------
%---------------------------------------------------------------------------------
%---------------------------------------------------------------------------------

% With mode  2    -- reference not interpolated in the same way
% --------------------------------------------
...
% default_limit_options = {[0 0.05],[0 8],[0 15],[0 1]};  %
default_limit_options = find_axis_limits(mode_data_str);
if isfield (mode_data_str,'reference_name'); reference_name = mode_data_str.reference_name; else reference_name = 'Disperse'; end %
mesh_names = create_mesh_names(mode_lookup);
...
'reference_name',reference_name,'mesh_names',{mesh_names},'allowable_angle',89.8));
...
%plot_data_structure.freq_for_convergence
...
%---------------------------------------------------------------------------------
% Why is it not plotting all thye modes at high frequency     ???????????????????
% Why is it not plotting all thye modes at high frequency     ???????????????????
% Why is it not plotting all thye modes at high frequency     ???????????????????
%---------------------------------------------------------------------------------
...
set(plot_data_structure.convergence_axis_handle,'XTick',1:length(plot_data_structure.r_p_d_l))
set(plot_data_structure.convergence_axis_handle,'FontSize',12)
hold on
...
h_conv{index} = plot(mesh_nos(find(~isnan(safe_mode_percent{index}) ~= 0)) , safe_mode_percent{index}(find(~isnan(safe_mode_percent{index}) ~= 0)),'-o','color',map_2(floor(index*length(map_2)/length(modes_in_scope_names)),:),...
'LineWidth',3,'MarkerEdgeColor',map_2(floor(index*length(map_2)/length(modes_in_scope_names)),:),'MarkerFaceColor',map_2(floor(index*length(map_2)/length(modes_in_scope_names)),:), 'MarkerSize',6);
%set(h_conv{index},'linewidth',4 )
...
set(mesh_convergence_TextBox,'position',[1000 660 420 160])
...
set(mesh_convergence_TextBox,'String',[{'Mesh Details:'},plot_data_structure.short_mesh_text',{''},{['Reference model: ',plot_data_structure.reference_name,'.']} ]);                      
...
plot_data_structure                               =          get(get(hObject,'Parent'),'UserData');
[plot_data_structure.lite_index,ok_]                =          listdlg('PromptString','Select mesh to display:','SelectionMode','single','ListString',arrayfun(@num2str,(1:size(plot_data_structure.r_p_d_l, 2)),'unif',0));
...
set(meshTextBox,'String',{[plot_data_structure.mesh_names{plot_data_structure.lite_index}],['Number of elements: ',num2str(length(reshaped_proc_data_lite.mesh.el.nds))],['Number of nodes: ',...
...
%************************
h_temp_a = title([plot_data_structure.mesh_names{plot_data_structure.lite_index},'  vs  ',plot_data_structure.reference_name,'(reference).']);
...
regroup_cell_string_array(plot_data_structure.modes_in_scope_display_text,2),{[' '],['Mesh ',num2str(plot_data_structure.lite_index),' Modes: ']},...
...
[angle_degrees,temp_max_ind,temp_min_ind] = find_gradiant_angle (mode_data_str.data{dis_mode_order(dis_modes_to_plot(index))}(:,plot_data_structure.x_index),...
    mode_data_str.data{dis_mode_order(dis_modes_to_plot(index))} (:,plot_data_structure.y_index) , freq_for_convergence);

if  ~isnan(angle_degrees)
if angle_degrees < plot_data_structure.allowable_angle    
counter_ = counter_+1;
modes_in_scope_indices (counter_) = index;
min_ind(counter_) = temp_min_ind;
max_ind(counter_) = temp_max_ind;

end % if angle_degrees < 85    
...
modes_in_scope_interp_y_ordinate_val{index} = interp1( mode_data_str.data{dis_mode_order(dis_modes_to_plot(modes_in_scope_indices(index)))}(min_ind(index):max_ind(index),plot_data_structure.x_index),mode_data_str.data{dis_mode_order(dis_modes_to_plot(modes_in_scope_indices(index)))}(min_ind(index):max_ind(index),plot_data_structure.y_index),freq_for_convergence, 'PCHIP');
...
else %if length(plot_data_structure.modes_in_scope_names)==0
...
%-----------------------------------------------------------------------------------------------------------------------
% only do this if it passes the gradiet test, otherwise make value NaN
%-----------------------------------------------------------------------------------------------------------------------

[angle_degrees,max_ind,min_ind] = find_gradiant_angle (current_freq_vals_temp , current_y_ord_vals_temp , freq_for_convergence);

if ~isnan(angle_degrees)
if angle_degrees < plot_data_structure.allowable_angle        
safe_interpolated_y_ord_vals_in_scope{mode_index}(mesh_index,pair_index)   = interp1(current_freq_vals_temp(min_ind:max_ind) , current_y_ord_vals_temp(min_ind:max_ind) , freq_for_convergence,'PCHIP');
else
safe_interpolated_y_ord_vals_in_scope{mode_index}(mesh_index,pair_index)   = NaN;    
end %if angle_degrees <plot_data_structure.allowable_angle    

else
safe_interpolated_y_ord_vals_in_scope{mode_index}(mesh_index,pair_index)   = NaN;
end %if ~isnan(angle_degrees)
...
short_mesh_text{index}  =  [ plot_data_structure.mesh_names{index},': # elements = ',num2str(mesh_stats_temp.no_of_elements),', mean length = ',num2str(round(mesh_stats_temp.mean_len*1000000)/1000),' mm.']; 
...
function  mesh_names = create_mesh_names(mode_lookup)

mesh_names = cell(1, size(mode_lookup.mode_indices{1},1)) ;
if isfield (mode_lookup,'mesh_name'); name_exist = 1 ; else  name_exist = 0 ; end

for index = 1: size(mode_lookup.mode_indices{1},1)
if name_exist==1
mesh_names{index} =  ['Mesh ',num2str(index),'(', mode_lookup.mesh_name{index},')'];
else
mesh_names{index} = ['Mesh ',num2str(index)];
end %if name_exist==1

end %for index = 1: size(mode_lookup.mode_indices{1},1)

end %function  mesh_names = create_mesh_names(mode_lookup);

function default_limit_options = find_axis_limits(mode_data_str)
% default_limit_options = {[0 0.05],[0 8],[0 15],[0 1]};  % should work out consistant way of determining these
% {[L_(1) H_(1)],[L_(2) H_(2)],[L_(3) H_(3)],[L_(4) H_(4)]}
...
temp_max =    max(mode_data_str.data{index});
temp_min =    min(mode_data_str.data{index});

for index_2 = 1:4
if temp_min(index_2) < L_(index_2)
    L_(index_2) = temp_min(index_2);
end

if temp_max(index_2) > H_(index_2)
    H_(index_2) = temp_max(index_2);
end
end %for index_2 = 1:4
...
%start by using the mins and max values

default_limit_options = {[L_(1) H_(1)],[L_(2) H_(2)],[L_(3) H_(3)/80],[L_(4) H_(4)]};

end %function default_limit_options = find_axis_limits(mode_data_str)

function [angle_degrees,max_ind,min_ind] = find_gradiant_angle ( freq_vals ,y_vals, chosen_freq)
find_gradient  = 1; 
...
if max(freq_vals) < chosen_freq || min(freq_vals) > chosen_freq
...
end%if ~isempty(close_to_chosen_value)   
      
end %if max(freq_vals) < chosen_freq || min(freq_vals) > chosen_freq   
...
if min_index   > length (freq_vals)-5  
max_ind = length (freq_vals);
else
max_ind = min_index+5;
end
angle_degrees = abs(180/pi*atan((spline(freq_vals(min_ind:max_ind)  ,y_vals(min_ind:max_ind)   ,freq_vals(min_index)) - spline(freq_vals(min_ind:max_ind),y_vals(min_ind:max_ind),freq_vals(min_index)+gap))/(gap*10 ))); 
