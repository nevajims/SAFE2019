...
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
% still need to be able to select from the graph
% option for plotting axis
% spline interpolation between points
% why are the 2d animations so jumpy  (when the 3d animation is smooth)
% some kind of visual display of the mode amplitude  (x/y   +  z)
% -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
% main program
% ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------


function [] =   plot_specific_mode_new(reshaped_proc_data)
...
'mode_index',5,'reshaped_proc_data',reshaped_proc_data,'xlim_',[0 , 0.1E5],'ylim_',[0 , 10000],'xlabel_','Freq(Hz)','ylabel_','Phase Velocity(m/s)',...
'cartesian_or_cylindrical',1,'mode_shape_display_option',1,'animate_button_value',0,'animation_timer_handle',-1,'current_animation_angle',0,'mult_factor_speed',30,...
'modes_to_plot',1:size(reshaped_proc_data.freq,2),'show_mesh',0));
...
%--------------------------------------------------------------------------------------------------------
% buttons, sliders etc-----------------------------------------------------------------------------------
slider_handle = uicontrol('Style', 'slider', 'Min',1,'Max',2,'Value',1,'Position'      , [20 20 120 20]     ,  'Callback' , @slider_func_1)  ; % controls magnification of animation
slider_handle_2 = uicontrol('Style', 'slider','Min',10,'Max',80,'Value',30,'Position'  , [960 440 120 20]   ,  'Callback' , @slider_func_2)  ; % controls speed of animation
...
fig_setup(fig_handle,slider_handle,slider_handle_2,checkbox_handle_mesh);                           
%------------------------------------------------------------------------------------------------------
...
function fig_setup(fig_handle,slider_handle,slider_handle_2,checkbox_handle_mesh)
% this is an 'autocallback'
plot_data_structure                    = get(fig_handle,'UserData') ;
size(plot_data_structure)
...
end %function slider_func_1 (hObject, ~) magnification of mesh displacements
...
end %function slider_func_2 (hObject, ~) speed of animation
...
axes(plot_data_structure.dispersion_axis);
[plot_data_structure.selected_freq,plot_data_structure.slected_pv, plot_data_structure.button_] = myginput(1,'crosshair');
...
% temp while sorting out the 3d animation -------------------------------
%if mode_shape_display_option == 3
%mode_shape_display_option = 2; % temp while static i sorted
%end %if mode_shape_display_option == 3
% temp while sorting out the 3d animation -------------------------------
...
set(mesh_axis,'xlim', animation_data_structure.xlim_ );
set(mesh_axis,'ylim', animation_data_structure.ylim_ );
set(mesh_axis,'zlim', animation_data_structure.ylim_ );
...
plot_data_structure = reset_mesh_plots(plot_data_structure);
set(get(hObject,'Parent'),'UserData',plot_data_structure);
end %chhose cartesian or cylindrical co ords for displacement calcs
...
selected_freq      =   plot_data_structure.selected_freq;
selected_pv        =   plot_data_structure.slected_pv;
...
min_index_v    = zeros(1,size(freq__,2));
norm_dist_vals = zeros(1,size(freq__,2));
...
freq    = plot_data_structure.reshaped_proc_data.freq(:,plot_data_structure.modes_to_plot)   ;
ph_vel   = plot_data_structure.reshaped_proc_data.ph_vel(:,plot_data_structure.modes_to_plot);

dispersion_axis = plot_data_structure.dispersion_axis;

xlim_    = plot_data_structure.xlim_;
ylim_    = plot_data_structure.ylim_;
xlabel_  = plot_data_structure.xlabel_;
ylabel_  = plot_data_structure.ylabel_;
...
axes(plot_data_structure.dispersion_axis)
plot(freq(point_index,mode_index) ,ph_vel(point_index,mode_index),'o','LineWidth',2,'MarkerEdgeColor','k','MarkerFaceColor','r','MarkerSize',10);
...
num2str(  round(10*freq(point_index,mode_index)/1000)/10  ),' kHz, Vph = ', num2str(round(10*ph_vel(point_index,mode_index)/1000)/10) ,' m/ms, Vgr = ', num2str(round(2*pi*10*gr_vel(point_index,mode_index)/1000)/10) ,...
' m/ms, K = ', num2str(gr_vel(point_index,mode_index))],'FontSize',18,'color','r'); 
